...
def fetch(series):
    req = urllib.request.Request(FRED.format(urllib.parse.quote(series)),
                                 headers={"User-Agent":"market-watch/1.0"})
    with urllib.request.urlopen(req, timeout=30) as r:
        txt = r.read().decode()
    rd = csv.DictReader(io.StringIO(txt))
    dk, vk = rd.fieldnames[0], series if series in rd.fieldnames else rd.fieldnames[1]
    out = []
    for row in rd:
        x = (row.get(vk) or "").strip()
        if x in ("", "."): continue
        try:
            out.append((datetime.strptime(row[dk], "%Y-%m-%d").date(), float(x)))
        except ValueError:
            pass
    if not out: raise RuntimeError(f"No data for {series}")
    return out
